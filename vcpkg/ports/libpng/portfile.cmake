...
# Overlay libpng 端口（照搬 vcpkg 上游 baseline b1e15ef，port-version bump 到 1）。
#
# 背景（缓存）：CI 二进制缓存里存有 triplet 修复前产出的 armv7 libpng16d.a，
# cairo(gio 链接链) 链它是报
#   ld.lld: error: .../lib/libpng16d.a(...) is incompatible with aarch64linux
# bump port-version 命缓存 key 变化，强制重建 arm64 版本。
# 背景（架构）：与 pcre2/zlib 同源——cmake 型端口在 arm64-android 上有被 get_vars
# 错推 armv7 的风险，故 Android arm64 分支显式指定 NDK aarch64 编译器并强制 ABI。

set(LIBPNG_ANDROID_OPTIONS)
if(VCPKG_TARGET_IS_ANDROID AND VCPKG_TARGET_ARCHITECTURE STREQUAL "arm64")
    # -------------------------------------------------------------------------
    # Android arm64：强制 NDK aarch64，避免错推 armv7（同 zlib/pcre2 overlay）
    # -------------------------------------------------------------------------
    ...
    elseif(DEFINED ENV{ANDROID_NDK_HOME})
        ...
        message(FATAL_ERROR "libpng(android): 未设置 ANDROID_NDK_HOME / VCPKG_ANDROID_NDK")
    endif()
    file(GLOB _prebuilt_dir_list DIRECTORIES "${_ndk}/toolchains/llvm/prebuilt/*")
    if(NOT _prebuilt_dir_list)
        message(FATAL_ERROR "libpng(android): 在 ${_ndk}/toolchains/llvm/prebuilt 下找不到工具链目录")
    endif()
    ...
    list(APPEND LIBPNG_ANDROID_OPTIONS
